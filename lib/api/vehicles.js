...
const getAllVehiclesData = () => {
  ...
  // Allow local updates/deletes to override mock data without duplication.
  const localMap = new Map(
    localVehicles.map((vehicle) => [String(vehicle.id), vehicle]),
  )
  const merged = MOCK_VEHICLES.map(
    (vehicle) => localMap.get(String(vehicle.id)) || vehicle,
  )
  const extra = localVehicles.filter(
    (vehicle) =>
      !MOCK_VEHICLES.some(
        (mockVehicle) => String(mockVehicle.id) === String(vehicle.id),
      ),
  )

  return [...merged, ...extra].filter((vehicle) => !vehicle.isDeleted)
}
...
export const vehicleAPI = {
  ...
  // Update a vehicle (stored locally to simulate persistence)
  updateVehicle: async (vehicle) => {
    ...
    const existingIndex = localVehicles.findIndex(
      (item) => String(item.id) === String(vehicle.id),
    )
    if (existingIndex >= 0) {
      localVehicles[existingIndex] = {
        ...localVehicles[existingIndex],
        ...updatedVehicle,
      }
    } else {
      localVehicles.unshift(updatedVehicle)
    }
    saveLocalVehicles(localVehicles)
    return { success: true, data: updatedVehicle }
  },

  // Delete a vehicle (uses a local tombstone for mock records)
  deleteVehicle: async (vehicleId) => {
    ...
    if (existingIndex >= 0) {
      localVehicles.splice(existingIndex, 1)
      saveLocalVehicles(localVehicles)
      return { success: true }
    }
    const tombstone = {
    ...
    saveLocalVehicles([tombstone, ...localVehicles])
    return { success: true }
  },
